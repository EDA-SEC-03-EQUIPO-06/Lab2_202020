...
moviesfile = "Data/Movies/SmallMoviesDetailsCleaned.csv"
...
def test_loading_CSV_y_ordenamiento(criteria):
    """
    ...
    while not (lt.isEmpty(lst_movies)):
        x = int(lt.removeLast(lst_movies)[criteria])
        if not (lt.isEmpty(lst_movies)):
            y = int(lt.lastElement(lst_movies)[criteria])
        else:
            break
        assert x > y
